...
export class GeminiProvider extends BaseProvider {
  ...
  private async *streamChat(options: ChatOptions): AsyncGenerator<SSEChunk> {
      ...
      formData.append("at", (this.config.authData.snlm0e as string) || "");

      const response = await fetch(
        `${this.baseUrl}/_/BardChatUi/data/assistant.lamda.BardFrontendService/StreamGenerate`,
        {
          method: "POST",
          headers: {
            ...this.headers,
  ...
  async fetchModels(): Promise<ProviderModel[]> {
    const { fetchGeminiModels } = await import("@/lib/models/fetch-official");
    return fetchGeminiModels(this.config, this.baseUrl);
  }
}
